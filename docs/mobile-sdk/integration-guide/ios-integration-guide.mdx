...
This document provides a step by step guide on how to install, integrate and initialize the BlockID SDK into any iOS applications.
...
This section explains the step by step guide on how to setup and integrate the BlockID SDK in an iOS application. 

:::tip 
The examples below show the integration of the BlockID SDK into a new iOS project - the same steps can be used to integrate the SDK into an existing application
:::
...
<img src={iOS3} alt='xcode project' />
...
<img src={iOS11} alt='install cocoapods' />

#### 2.2 Construct the Podfile

Now that you have installed the CocoaPods, next step is to create an empty file in your Xcode project directory named `Podfile`
...
<img src={iOS14} alt='Install Podfile' />
...
``` 
- Copy the entire `BlockIDSDK.framework` folder to your *BlockID Demo* root diectory

import iOS17 from '../assets/iOS-17.png';
<img src={iOS17} alt='BlockID SDK.framework' />
...
<img src={iOS18} alt='Embed and Sign' />
...
<img src={iOS19} alt='additional pods' />

### Step 4: Enable Bitcode

For iOS apps, bitcode is the default, but optional. The BlockID SDK supports enabling bitcode option in the application. 

### Step 5: Applicaiton Deployment 

iOS applications require proper Xcode configuration. As the current version of BlockID iOS SDK (`version 1.6.00`) is *FAT framework*, a workaround is provided below for an `iOS App Store` submission bug that that appears when archiving universal binaries is required on your project. 
...
:::tip
Encryption of data takes longer while the app runs in *Debug* mode. It is recommended (but not mandatory) to change the application scheme to *Relese* before running the application
:::
